...
To handle spatial trancriptomics technololgies in the Bioconductor project it's 
possible to find multiple approaches.
The SpatialExperiment infrastructure [ref] allows to handle both technology 
families by aid of the `SpatialExperiment` class, allowing to easily store and 
manipulate the data.
The TENxVisiumData package provides a collection of 13 datasets from
...
hbc <- spatialPreprocess(hbc, platform="Visium", n.PCs=7, n.HVGs=2000)
...
hbc <- qTune(hbc, qs=seq(2, 10), platform="Visium", d=7)
qPlot(hbc)
