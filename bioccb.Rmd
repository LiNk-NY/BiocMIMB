...
spot-based (i.e. 10x genomics Visium [ref], Slide-seq [ref]) and molecule-based (i.e. MERFISH[ref], seqFISH(+) [ref], CosMx [ref]), . The first one collects, together with the expression of the transcripts the coordinates of barcoded spots, which, depending on the technology could be represented by a small hole on a slide where the cells falls or a bead where the transcripts are ligate.
On the other hand, the latter family detects pre-defined panel of transcripts in-loco by combining fluorescent probes through the FISH technique.
In both cases the number of trascripts depends on the adopted technology.
To handle spatial trancriptomics technololgies in the Bioconductor project it's possible to find multiple approaches  
