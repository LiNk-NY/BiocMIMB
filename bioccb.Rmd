...
The Bioconductor project offers multiple approaches to handle and manupulate  
spatial transcriptomics (ST) data.
The SpatialExperiment infrastructure [ref] allows to handle different technology
families by aid of the `SpatialExperiment` class, which hinerits from the SingleCellExperiment class, allowing to easily store and manipulate such data.
The same infrastructure provides the ggspavis package for the data visualization and two ExperimentHub packages allowing to download
multiple ST datasets. 
The STexampleData is a collection of small dataset of different technologies mostly of human and mouse brain tissue.
...
official 10xGenomics website, of which 5 are related to cancer.
In particular, we recognize 2 Human Breast Cancer (retrievable with `HumanBreastCancerIDC`, `HumanBreastCancerILC`), 1 Human Colorectal Cancer (`HumanColorectalCancer`), 1 Human Glioblastoma (`HumanGlioblastoma`) and 1 HumanOvarianCancer (`HumanOvarianCancer`).
...
To investigate the spatially variable genes the nnSVG package in Bioconductor 
...
We now visualize the most spatially variable gene identified by nnSVG.
...
The SpatialFeatureExperiment [ref] is a class inherithing from the 
...
We specifically recognize 4 datasets related to cancer, melaanoma brain and extracranial metastasis (`BiermannMelaMetasData`), human breast cancer (`JanesickBreastData`) and
lung cancer (`HeNSCLCData`), 

Here we show how the data of the breast cancer dataset are distributed and how this expression varies in the tissue.
