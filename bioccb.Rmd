...
library(nnSVG)
## needed because the column name is hard coded in the nnSVG::filter_genes function 
...
spe <- spe[order(rowData(spe)$rank),]
# spe <- readRDS("~/Downloads/spennsvg.Rds")
library(ggspavis)
plotSpots(spe)
gnr1 <- rowData(spe)$symbol[1]
...
# # bayesspace works with SingleCellExperiment with coordinates as row 
# # and col in colData
# gbm <- HumanGlioblastoma()
# table(gbm$sample_id)
# pos <- read.csv("/Users/inzirio/Library/CloudStorage/GoogleDrive-drighelli@gmail.com/My Drive/works/PABSTR/papers/BiocMIMB/spatial/tissue_positions_list.csv", header=FALSE)
# colnames(pos) <- c("barcode", "in_tissue", "array_row", "array_col", "pxl_row_in_fullres", "pxl_col_in_fullres")
# pos <- pos[pos$in_tissue,]
# pos$barcode
# idx <- match(colnames(altExp(gbm)), pos$barcode)
# 
# # length(idx)
# sum(is.na(idx))
# # 
...
# urls
# # 
# # hbc$row <- rank(spatialCoords(hbc)[,2])
# # hbc$col <- rank(spatialCoords(hbc)[,1])
# # hbc$imagerow <- spatialCoords(hbc)[,2]
# # hbc$imagecol <- spatialCoords(hbc)[,1]
# set.seed(102)
# melanoma <- spatialPreprocess(melanoma, platform="ST", 
#                               n.PCs=7, n.HVGs=2000, log.normalize=FALSE)
# hbc <- qTune(melanoma, qs=seq(2, 10), platform="ST", d=7)
# qPlot(hbc)
# melanoma <- hbc
# set.seed(149)
...
# # hbc <- spatialCluster(hbc, q=9, platform="Visium", d=7,
#                            init.method="mclust", model="t", gamma=2,
#                            nrep=1000, burn.in=100,
#                            save.chain=TRUE)
# table(hbc$cluster.init, hbc$spatial.cluster)
# clusterPlot(hbc)
