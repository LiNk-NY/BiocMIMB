...
tcgatab = curatedTCGAData()
```
The first 10 records have:
```{r lktab,echo=FALSE}
kbl(head(tcgatab[,1:4],10), booktabs=TRUE)
```
...
`[tumorcode]_[assay]-[date]_[optional codes]`.
...
```{r docodes,echo=FALSE}
kbl(cotab, booktabs=TRUE)
```

### Assay codes and counts

```{r doassc,echo=FALSE}
kbl(assaycounts, booktabs=TRUE)
```
