...
author: 
  - name: Dario Righelli
    affiliation:  Department of Statistical Sciences, University of Padova, Padova, IT
    orcid: 0000-0003-1504-3583
  - name: Davide Risso
    affiliation:  Department of Statistical Sciences, University of Padova, Padova, IT
    orcid: 0000-0001-8508-5012
  - name: Marcel Ramos
    affiliation:  City University of New York School of Public Health, New York, NY
  ...
  - name: Alexandru Mahmoud
    affiliation: Channing Division of Network Medicine, Mass General Brigham, Harvard Medical School, Boston, MA
    orcid: 0000-0002-3779-492X
  - name: Hervé Pagès
    affiliation: Fred Hutchinson Cancer Center, Seattle, WA
  - name: Jen Wokaty
    affiliation:  City University of New York School of Public Health, New York, NY
  - name: Sean Davis
    affiliation: University of Colorado Anschutz School of Medicine, Aurora, CO
    orcid: 0000-0002-8991-6458
  - name: Sehyun Oh
    affiliation:  City University of New York School of Public Health, New York, NY
    orcid: 0000-0002-9490-3061
  - name: Levi Waldron
    affiliation:  City University of New York School of Public Health, New York, NY
...
abstract: >
  ...
  to researchers. Over 60,000 PubMed Central
  citations and terabytes of content shipped per month
  attest to the impact of the project on cancer genomic data science. This report
  provides an overview of cancer genomics resources in Bioconductor.
  After an overview of Bioconductor project principles,
  we address exploration of institutionally curated cancer
  genomics data such as TCGA. We then review genomic annotation and ontology
  resources relevant to cancer and then briefly survey
  Analytical workflows addressing specific topics in cancer
  genomics.  Concluding sections cover
  how new software and data resources are brought into the
  ecosystem and how the project is tackling needs for
  training of the research workforce.
  Bioconductor’s strategies for
...
load("datadesc.rda")
load("datadesc2.rda")
datadesc2[48,2] = gsub("display", "display ", datadesc2[48,2])
datadesc2[49,2] = gsub("Mut", " Mut ", datadesc2[48,2])
load("easvig.rda")
load("wflowtab.rda")
...
## Downloading and caching cancer genomics data and annotations {#cache}
...
<!-- , fig.cap="ExperimentHub attachment, retrieval, query, and response when seeking cancer-related data.", message=FALSE} -->
```{r useeh} 
...
kbl(tcgatab[grep("ACC", tcgatab$title),1:4], booktabs=TRUE, caption="Records returned by curatedTCGAData::curatedTCGAData(), filtered to those pertaining to adrenocortical carcinoma.")
```
...
```{r strat, fig.cap="Survival distributions for donors of breast tumors in TCGA, stratified by presence or absence of mutation in gene TTN."}
...
kbl(lk3[,4:6], booktabs=TRUE) |> column_spec(2, width="15em") |> column_spec(3,
  width="15em")
```
...
[@Tian2018]
and esATAC [@Wei2018], which start from fastq files and produce feature count
...
provides
a variety of QC plots specific to ATAC-seq data [@Ou2018].

For DNA methylation, many popular packages have been developed to help with
all stages of a DNA methylation analysis. These include minfi [Aryee@2014],
which specializes in methylation array analysis, biseq and bsseq [@Hansen2012] 
which provide fundamental infrastructure for sequencing-based assays, and RnBeads
[@Mueller2019], which provides a comprehensive general-purpose analysis of DNA
methylation cohorts from arrays or sequencing-based assays. Other packages provide more specialized
analysis approaches, such as MIRA [@Lawson2018], which  infers regulatory
...
cohorts to infer transcription factor networks [@Silva2019]. EpiDISH infer
the proportions of cell-types present in a bulk sample on the basis
of DNA methylation data [@Zheng2018a].

Another popular epigenome experiment is ChIP-seq, and Bioconductor delivers many packages in
this area. DiffBind [@Stark2011] is a popular approach for
...
of data. GenomicDistributions [@Kupkova2022] provides a variety of plots for visualization
...
unsupervised exploration of variation in epigenomic data. PathwayPCA, MOFA2 [@Argelaguet2020]
and COCOA [@Lawson2020] can process any epigenomic signal data.
A variety of alternative approaches for enrichment analysis, which include LOLA [@Sheffield2016],
chipenrich, regionR [@Gel2015], and FGNet [@Aibar2015].
Annotation packages are popular as well. ChIPpeakAnno [@Zhu2010]
and annotatr [@Cavalcante2017] are popular packages for annotating genomic
ranges.
...
## Representing and visualizing spatial transcriptomics experiments

<!-- Spatial Transcriptomics experiments can be divided in two major families, -->
<!-- spot-based (i.e. 10x genomics Visium [ref], Slide-seq [ref]) and molecule-based  -->
<!-- (i.e. MERFISH[ref], seqFISH(+) [ref], CosMx [ref]), . The first one collects,  -->
<!-- together with the expression of the transcripts the coordinates of barcoded  -->
<!-- spots, which, depending on the technology could be represented by a small hole -->
<!-- on a slide where the cells falls or a bead where the transcripts are ligate. -->
<!-- On the other hand, the latter family detects pre-defined panel of transcripts  -->
<!-- in-loco by combining fluorescent probes through the FISH technique. -->
<!-- In both cases the number of trascripts depends on the adopted technology. -->
<!-- To handle spatial trancriptomics technololgies in the Bioconductor project -->
<!-- it's possible to find multiple approaches. -->
The Bioconductor project offers multiple approaches to handle and manipulate  
spatial transcriptomics (ST) data.
The SpatialExperiment infrastructure [@rig22] allows to handle different technology
families by aid of the `SpatialExperiment` class, which inherits from the 
SingleCellExperiment class, allowing to easily store and manipulate such data.
The same infrastructure provides the ggspavis package for the data visualization 
and two ExperimentHub packages allowing to download
multiple ST datasets. 
The STexampleData is a collection of small datasets of different technologies 
mostly of human and mouse brain tissue.
While the TENxVisiumData package provides a collection of 13 datasets from
23 samples across two organisms (human and mouse) and 13 tissues from the 
official 10x-Genomics website, of which 5 are related to cancer.
In particular, we recognize 2 Human Breast Cancer (retrievable with `HumanBreastCancerIDC`, `HumanBreastCancerILC`), 1 Human Colorectal Cancer (`HumanColorectalCancer`), 
1 Human Glioblastoma (`HumanGlioblastoma`) and 1 HumanOvarianCancer (`HumanOvarianCancer`).

Here we use the `HumanBreastCancerIDC` dataset by selecting one of the two
samples and we plot the coordinates, as spots, with the ggspavis package.
...
hbc$in_tissue =TRUE
plotSpots(hbc, y_reverse=FALSE)
```


<!-- We notice that the image is rotated respect to the spots coordinate so we can  -->
<!-- rotate it thanks to SpatialExperiment features -->
<!-- ```{r} -->
<!-- plot(imgRaster(hbc)) -->
<!-- spi <- rotateImg(hbc, "HumanBreastCancerIDC1", "lowres", -90) -->
<!-- plot(imgRaster(spi)) -->
<!-- ``` -->

To investigate the spatially variable genes the nnSVG package [@webr23] in Bioconductor 
implements a method for the detection of genes which expression varies in the
...
# hbc <- readRDS("~/Downloads/spennsvg.Rds")
...
variable genes. 
Here we show how the top identified gene spatially varies across the spots in
our experiment.

```{r}
library(ggplot2)
df <- as.data.frame(
  cbind(spatialCoords(hbc), 
        expr = counts(hbc)[1, ]))
...
The SpatialFeatureExperiment [@moses23] is a class inheriting from the 
SpatialExperiment class, by integrating `sf` objects useful for spatial geometry 
and statistics already implemented in the CRAN sf package.

It also provides 9 datasets from 6 different technologies and tissues in the
SFEData package.

We specifically recognize 4 datasets related to cancer, melaanoma brain and 
extracranial metastasis (`BiermannMelaMetasData`), human breast cancer 
(`JanesickBreastData`) and
lung cancer (`HeNSCLCData`), 

Here we show how the data of the breast cancer dataset are distributed and how 
this expression varies in the tissue.
...
# Components and processes for introducing new data, analytic tools, documents {#class}
...
A MultiAssayExperiment instance `M` can be
filtered as a three-dimensional array.
When `G` is a vector of feature identifiers,
`C` a vector of sample identifiers, and `E` a
vector of experiment names, then `M[G, C, E]` is
...
## Out-of-memory data representation strategies
...
```{r dotumm, message=FALSE}
library(TumourMethData)
demm = download_meth_dataset("mcrpc_wgbs_hg38_chr11")
demm
rowRanges(demm)
names(colData(demm))
...
References to `demm` involve an 800MB excerpt of a
...
```{r getatm, message=FALSE}
library(EnsDb.Hsapiens.v86)
gg = genes(EnsDb.Hsapiens.v86)      # get gene addresses
atmpos = gg[gg$gene_name == "ATM" &
    gg$gene_biotype == "protein_coding"] # filter to ATM
seqlevelsStyle(atmpos) = "UCSC"     # translate chr names
assay(subsetByOverlaps(demm, atmpos+1e6))  # get betas
```
...
## Quality assessment of Bioconductor resources
...
```{r qapic, echo=FALSE, fig.cap="Build report for Bioc 3.18, 12-29-2023.", out.width="121%"}
knitr::include_graphics("QApage.jpg")
```
...
- **Partnering with The Carpentries.**  The Carpentries \url{https://carpentries.org} is a non-profit organization focused on teaching programming
...
- **Curating monographs for topics in genomic data science.**  The
...
- **Language-agnostic data and annotation**  The `alabaster.*` packages
...
by software in other languages.  Thus the `alabaster.mae`
...
- **Zero-configuration genomic analysis environments**  Users
...
- **Computation at the data**  Several members of Bioconductor's
...
strategy is described in @Schatz2022.  This system was
used in the production of the Telomere-to-Telomere
genome build, see @Aganezov2022.
...
kbl(as.data.frame(candesc2), booktabs=TRUE, longtable=TRUE) %>% column_spec(2, width="25em") %>%
 kable_styling("repeat_header")
...
kbl(as.data.frame(datadesc2), booktabs=TRUE, longtable=TRUE) %>% column_spec(2, width="25em") %>%
 kable_styling("repeat_header")
```
