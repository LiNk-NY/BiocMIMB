...
"SpatialExperiment", "TENxVisiumData", "ggspavis", "nnSVG", "scater", "scran",
"Voyager", "scattermore", "gridExtra")
...
decoding of tumor codes in alphabetical order is
...
all stages of a DNA methylation analysis. These include minfi [@Aryee2014],
...
The SpatialExperiment class [@rig22] is designed to be a lightweight,
...
SpatialFeatureExperiment [@moses23] extends SpatialExperiment to easily 
reuse polygons and other spatial geometry features from geospatial CRAN 
packages, such as sf. See also MoleculeExperiment [@Couto2023] for a different
...
```{r tenxvisium, fig.cap="Visualization of a Visium breast cancer sample"}
...
tissue spatial domains by fitting nearest-neighbor Gaussian processes [@webr23].
...
```{r plotvisium, fig.cap="Spatial expression of a highly variable gene"}
...
```{r plotvoyager, fig.cap="Cell density and cell boundaries of a Xenium breast cancer sample"}
library(Voyager)
...
```{r sfemark, fig.cap="Spatial expression of marker genes"}
...
\url{https://github.com/bioconductor/contributions/issues}.  Contributors
