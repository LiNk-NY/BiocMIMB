...
"BiocStyle", "ensembldb", "rols", "GO.db",
...
table(genes(EnsDb.Hsapiens.v86)$gene_biotype)
```
...
```{r trymsig,message=FALSE,cache=TRUE}
library(msigdb)
hsall = getMsigdb(
       org = "hs",
       id = c("SYM", "EZID"),
...
```{r lknms}
nms = grep("ANCER", names(hsall), value=TRUE)
head(nms)
wangmet = hsall[["WANG_METASTASIS_OF_BREAST_CANCER_ESR1_UP"]]
wangmet
```
...
but frequently ambiguous terminology.  
In modern information science, ontologies are structured vocabularies
...
in cancer genomics data science.  The most familiar ontology
...
Once the formal identifier is found, the org.Hs.eg.db package
...
```{r spatchunk, message=FALSE,cache=TRUE}
